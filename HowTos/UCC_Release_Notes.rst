...
- **Transit BGP over GRE Tunnel** provides alternative tunneling protocol to IPSec when connecting Aviatrix Transit Gateway to on-prem. One use case is to improve performance comparing to using IPSec. Only available in AWS (Azure and GCP do not support GRE). For configuration information, refer to `Aviatrix Transit Gateway to External Devices <https://docs.aviatrix.com/HowTos/transitgw_external.html#how-to-configure>`_.

- **Transit BGP to LAN** allows Aviatrix Transit Gateway to deploy BGP and data plane with an instance in the same VPC or VNet without running a tunnel protocol such as IPSec and GRE. One use case is to interoperate with third party virtual appliances such as a SD-WAN cloud instance that do not have the capability to support BGP over any tunneling protocols. For configuration information, refer to `Aviatrix Transit Gateway to External Devices <https://docs.aviatrix.com/HowTos/transitgw_external.html#how-to-configure>`_.
