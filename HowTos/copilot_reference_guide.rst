...
CostIQ offers visibility into costs that are generated by your enterprise business units and departments (cost centers) for resources they use across all clouds in your multi-cloud network that is managed by Aviatrix Controller. 

In CostIQ, you define your cost centers and any resources they share (shared services) in your network. Cost centers can help you identify the top utilized VPC/VNets by each of your cost centers. The traffic breakdown information is useful for Finance teams for billing purposes. 

CostIQ uses the information to report on which cost centers use the most resources, identifying the top utilized VPC/VNets by each of your business units/departments.  Costs are reported as percentages of traffic generated by end instances in spoke VPC/VNets. 

To use CostIQ, you first create the following constructs:

-   Cost centers defined by a set of VPC/VNets. The VPC/VNets are those that contain resources used by your real life cost centers or business units. CoPilot looks at the traffic generated by all end instances in the VPC/VNets and calculates the percentage of traffic they generate when compared to the total traffic for all end instances in all VPC/VNets in your Aviatrix managed environment. 

-   Shared services defined by the IP addresses or CIDR ranges of resources in your network shared by multiple departments, such as an S3 bucket share by more than one organization.




Interacting with CostIQ
------------------------

Click segments of the doughnut charts to drill down into more granular detail about the traffic.

Use filters in the Cost Details panel to filter information about traffic going to your shared services. 

-   Filter by cloud account

    Show details for all traffic going to defined shared services from instances/hosts owned by a specific cloud account. CoPilot considers the VPC cloud account (not the spoke gateway cloud account). The traffic breakdown shows the relative percent of traffic and the absolute value of the traffic.

-   Filter by cost center

    Show details for all traffic going to defined shared services from instances/hosts owned by a specific cost center. The traffic breakdown shows the relative percent of traffic and the absolute value of the traffic.

-   Filter by time period

    Use the time selector to filter the traffic information by a specified time period. 
 

Create a Cost Center in CostIQ 
-------------------------------

In CostIQ, you define *cost center* constructs that represent your real-life cost centers (business units, departments, teams).  

Cost centers are defined by a set of VPC/VNets. 

You can edit the name of a cost center as well as the list of VPC/VNets that define the cost center at any time.

Delete a Cost Center from CostIQ
---------------------------------

You can delete a cost center at any time. 

You can reconstruct a cost center using the same set of spoke gateways at any time.

CoPilot considers only the current set of VPC/VNets when calculating total traffic percentages. The traffic for a deleted VPC/VNet is no longer considered in the calculation. A deleted VPC/VNet is not automatically removed from a cost center; it remains in the cost center but is no longer factored into the calculation of total traffic percentages.

Create a Shared Service in CostIQ
----------------------------------

In CostIQ, you define *shared service* constructs that represent the resources used jointly be multiple cost centers (shared services).  

Delete a Shared Service from CostIQ
------------------------------------
...
The CostIQ overview shows a breakdown of your managed resources by the cost centers that generate the most expenditures.

Descriptions of the properties in the CoPilot CostIQ Overview (dashboard) listed in alphabetical order:

-   **Time Period**

    The time ranges you can set for viewing cost information of managed resources in cost centers.
...
-   **Time Period Start**
    ...
    Start date and start time of the time range for which cost information is shown for managed resources in cost centers.

-   **Time Period End**

    End date and end time of the time range for which cost information is shown for managed resources in cost centers.

-   **Top-Cost Cost Centers**

    Of all the cost centers you have defined, the ones that have the most expenditures.


**CostIQ Shared Services Properties**

Descriptions of the properties in the CoPilot CostIQ Shared Services tab listed in alphabetical order:

-   **Name**

    The name you assign to the shared service.

    For example, DevS3Bucket if multiple development teams are using an AWS S3 bucket.

    Allowed: Alphanumeric characters, whitespace, hyphens, underscore, and max-length 128 characters

    Not allowed: No Special Characters (Sorry Elon & X Æ A-12)

-   **IP CIDRs**

    The IP address or CIDR range of the shared resource(s).

**CostIQ Cost Centers Properties**

Descriptions of the properties in the CoPilot CostIQ Cost Centers tab listed in alphabetical order:

-   **Name**

    The name you assign to the cost center.

    For example, `Marketing` or `Engineering` if all of the spoke gateways in these cost centers run workloads for those departments.
...
-   **Percentage**
    ...
    The percentage (%) values reported are of the total traffic for *all* VPC/VNets within the specified time frame regardless of whether the VPC/VNets are associated with cost domains or not. For example, let's say you have VPC/VNets A, B, C, and D and define a cost domain with VPC/VNets A and B. If CoPilot reports **Prev Month** as 7%, this means in the previous month the traffic A + B is 7% of traffic A + B + C + D.
