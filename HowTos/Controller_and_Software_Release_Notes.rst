...
* **AVX-36880** - You can now upgrade images for multiple non-ActiveMesh Aviatrix Standalone Gateways in batches, instead of individually. This improvement makes the image upgrade process faster and more efficient for this type of gateway.
  ...
  You can upgrade non-Activemesh gateway images in batches if they have no peerings, or if only one of the gateways has a peering. If more than one non-Activemesh gateway has a peering, the batch image upgrade will fail.
...
* **AVX-40292** - (Azure) For any BGP over LAN peering connection with Route Approval, when you removed approval for the CIDR of the peered gateway, all routes were withdrawn.
* **AVX-40871** - Improved the Stateful Firewall data plane synchronization between the Primary and HA gateways.
* **AVX-41271** - The Stateful Firewall feature had duplicate entries in the data plane.
...
* **AVX-41271** - The Stateful Firewall feature had duplicate entries in the data plane.
* **AVX-41340** - (GCP) Upgrading a GCP Transit Gateway with BGPoLAN and Firenet features enabled might have resulted in a BGP session failure.
* **AVX-41489** - When the Controller Access feature was enabled, and the CoPilot IP address used was an FQDN, IPSET configuration failed.
* **AVX-41509** - 

  Conntrack-Limits Network Performance Metrics

  (AWS Cloud) You can now monitor the following conntrack-limits network performance metrics for Aviatrix Gateway instances:
 ...
 * Go to CoPilot > Monitor > Performance page.
 * Select the applicable gateway(s).
 * Select the Conntrack Allowance Available metric.
 * Select the Conntrack Usage Rate metric
 * Select the applicable ethx interfaces to monitor.
 * Click **Apply**.
